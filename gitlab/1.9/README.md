...
- [Jenkins](https://docs.mesosphere.com/1.9/usage/service-guides/jenkins/) must be installed and running
...
You will need to do this for the Jenkins agent too using the [Advanced Configuration instructions](https://docs.mesosphere.com/1.9/usage/service-guides/jenkins/advanced-configuration/).
...
- Finally, you will want to ensure your DC/OS agents are authenticated against this registry, [see the instructions](https://mesosphere.github.io/marathon/docs/native-docker-private-registry.html) on how to create the /etc/docker.tar.gz file with the registry credentials that will be expected.
...
```
    ...
    node {
        ...
        withCredentials(
            ...
            sh "docker login -u ${env.GITLAB_USERNAME} -p ${env.GITLAB_PASSWORD} gitlab-test.mesosphere.com:50000"
...
1. Navigate to Jenkins. First we'll add the GitLab username and password to the Jenkins Credentials store. You can do this by clicking on the Credentials link in the sidebar, then on the Global credentials domain and Add Credentials.

For now, we'll re-use the root user, but you can create new users as you see fit. Make sure to call these credentials `gitlab` as referenced in the Jenkinsfile:
