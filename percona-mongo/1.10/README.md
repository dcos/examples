...
| PRIMARY downtime when altering mongo configuration (e.g. version, cpus, etc.) | We would have liked to use one-shot tasks to step down a primary before it is updated, but this bug prevents us from doing this. As a workaround we use a watchdog to fix any bad mongo state. It will take some seconds for this watchdog to fire (and therefore reinstate a PRIMARY). | [DCOS_OSS-1544](https://jira.mesosphere.com/browse/DCOS_OSS-1544)                                                                 |
