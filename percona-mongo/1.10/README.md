...
| Config: XFS Formatting | Currently the framework is unable to enforce an XFS-based filesystem for storing MongoDB data. **We do not recommend WiredTiger-based installations *(the default)* run on filesystems other than XFS! Ensure DC/OS agent nodes have XFS filesystems.** | [mesosphere/dcos-mongo#29](https://github.com/mesosphere/dcos-mongo/issues/29) |
